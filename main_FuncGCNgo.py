...
from model.FuncGCNgo import FuncGCNgo
import time
import os
...
conjName=r"conj\FuncGCNgo.json"
...
def main():
    ...
    static_adj,node_features = build_GoStructure(args)

    logging.info("finish building adj %d sec" % (time.time() - start_time))
    ########### 2. 3. 训练 ###########
    model = FuncGCNgo(args).to(args["device"])
    ...
    for epoch in range(args["epochs"]):
        ...
        for batch,the_train_mask,idx in zip(dataloader, train_mask_loader,idex_loader):
            ...
            optimizer.zero_grad()
            out = model(batch_x,static_adj)#torch.Size([64, 589])
        ...
        if train_Loss < min_loss:
            min_loss = train_Loss
            best_epoch = epoch
            torch.save(model.state_dict(), f"experiments\\FuncGCNgo\\best_model_{best_epoch}.pth")
    ...
    best_model = FuncGCNgo(args)
    best_model.load_state_dict(torch.load(f"experiments\\FuncGCNgo\\best_model_{best_epoch}.pth"))
    ...
    for batch, the_test_mask , idx in zip(dataloader, test_mask_loader,idex_loader):
        ...
        out = best_model(batch_x, static_adj)  # torch.Size([64, 589])
