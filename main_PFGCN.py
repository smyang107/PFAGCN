...
from model.PFGCN import PFGCNgo,PGCN,FGCN,PFGCNgo_AA
...
def main():
    ...
    for epoch in range(args["epochs"]):
        ...
        if train_Loss < min_loss:
            min_loss = train_Loss
            best_epoch = epoch
            torch.save(model.state_dict(), f"experiments\\{model_type}\\{function}\\best_model_{best_epoch}.pth")
    ...
    best_model = PFGCNgo_AA(args)
    best_model.load_state_dict(torch.load(f"experiments\\{model_type}\\{function}\\best_model_{best_epoch}.pth"))
    ...
    # 1. 创建文件对象
    f = open('res/res_'+model_type+'_'+args["function"]+'.csv', 'w', encoding='utf-8', newline="")
    # 2. 基于文件对象构建 csv写入对象
