...
from model.ProteinGCN import ProteinGCNgo
import time
import os
...
def main():
    ...
    ########### 2. 3. 训练 ###########
    model = ProteinGCNgo(args).to(args["device"])
    ...
    for epoch in range(args["epochs"]):
        ...
        for batch,the_train_mask,idx in zip(dataloader, train_mask_loader,idex_loader):
            ...
            optimizer.zero_grad()
            out = model(batch_x,batchAdj)#torch.Size([64, 589])
        ...
        if train_Loss < min_loss:
            min_loss = train_Loss
            best_epoch = epoch
            torch.save(model.state_dict(), f"experiments\\ProteinGCNgo\\best_model_{best_epoch}.pth")
    ...
    best_model = ProteinGCNgo(args)
    best_model.load_state_dict(torch.load(f"experiments\\ProteinGCNgo\\best_model_{best_epoch}.pth"))
    ...
    for batch, the_test_mask , idx in zip(dataloader, test_mask_loader,idex_loader):
        ...
        batchAdj = adj_batches[i, :, :].clone().detach().to(args["device"])
        #args["supports"] = batchAdj
        out = best_model(batch_x, batchAdj)  # torch.Size([64, 589])
