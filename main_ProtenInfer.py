...
from model.ProteInfer import ProteInfer as Model
...
conjName=r"conj\protenInfer.json"
...
def read_config():
    ...
    config["nb_classes"] = len(set(functions))
...
def main():
    ...
    all_values, train_mask, val_mask, test_mask, shuffled_idx, gos, sequences, _ = load_sequences(DATA_ROOT=args["DATA_ROOT"],
    ...
    for epoch in range(args["epochs"]):
        ...
        if train_Loss < min_loss:
            min_loss = train_Loss
            best_epoch = epoch
            torch.save(model.state_dict(), f"experiments\\ProteInfer\\best_model_{best_epoch}.pth")
    ...
    best_model.load_state_dict(torch.load(f"experiments\\ProteInfer\\best_model_{best_epoch}.pth"))
    ...
    for batch, the_test_mask , idx in zip(dataloader, test_mask_loader,idex_loader):
        ...
        optimizer.step()
        i = i + 1
    f_max, p_max, r_max, t_max, predictions_max, recall_list, predictions_list = compute_performance(
        all_out.detach().cpu().numpy()
    ...
    roc_auc = compute_roc(all_out.detach().cpu().numpy(), test_y.detach().cpu().numpy(),
                          args)  # torch.Size([3141, 589]),(31530, 1) 多维数据的降维函数
    ...
    # 1. 创建文件对象
    f = open('res/res_ProtenInfer_'+args["function"]+'.csv', 'w', encoding='utf-8', newline="")
    # 2. 基于文件对象构建 csv写入对象
